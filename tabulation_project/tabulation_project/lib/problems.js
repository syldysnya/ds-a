...
//         let left = table[i - 2] === undefined ? 0 : table[i - 2];
//         let sumInclude = left + nums[i];
//         let sumNotInclude = table[i - 1];
//         table[i] = Math.max(sumInclude, sumNotInclude)
//     }
...
function minChange(coins, amount) {
    ...
    coins.forEach(val => { // val = 1
        for (let amt = 0; amt < table.length; amt++) { // amt = 0
            for (let qty = 0; qty * val <= amt; qty++) { // qty = 1; 1 * 1 <= 0
                remainder = amt - qty * val; // 1 - 0 * 1
                let attempt = table[remainder] + qty; // table[1] + 0

                if (attempt < table[amt]) table[amt] = attempt; // if 0 < 0
            }
        }
    });
